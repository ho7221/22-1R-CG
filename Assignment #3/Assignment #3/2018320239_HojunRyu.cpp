...
int WINDOW_WIDTH = 6000;
int WINDOW_HEIGHT = 3375;
...
vec3 get_reflection_ray(vec3 input_ray, HitData hit) {
	//TODO: Problem1
	return normalize(input_ray - 2.0f * hit.normal * dot(hit.normal, input_ray));
}
vec3 get_refraction_ray(vec3 input_ray, HitData hit, Material m) {
	...
	float refr_rate = n1 / n2;
	vec3 normal = -hit.normal;
	vec3 I_orth = dot(normal, input_ray) * normal;
	vec3 I_para = input_ray - I_orth;

	vec3 T_para = refr_rate * I_para;
	vec3 T_orth = float(sqrt(1 - pow(length(T_para), 2))) * normal;

	return normalize(T_orth + T_para);
}

bool is_lighted(vec3 eye) {
	//TODO: Problem3
	vec3 sray = LIGHT_POS - eye;

	HitData hit = get_closest_hit(eye, sray);
	if (hit.object_index < 0) return true;
	else return false;
}
void construct_normal_map(vector<vector<vec3>>& img_height, vector<vector<vec3>>& img_normal)
{
	...
	for (int x = 0; x < TEXTURE_IMAGE_WIDTH-1; x++)
	{
		for (int y = 0; y < TEXTURE_IMAGE_HEIGHT-1; y++)
		{
			vec3 dx, dy;
			if (x == 0) {
				dx = img_height[0][y] - img_height[1][y];
			}
			else if (x == TEXTURE_IMAGE_WIDTH - 2) {
				dx = img_height[TEXTURE_IMAGE_WIDTH - 2][y] - img_height[TEXTURE_IMAGE_WIDTH - 3][y];
			}
			else dx = img_height[x - 1][y] - img_height[x + 1][y];
			dx = vec3(2.0, 0.0, dx[2]);

			if (y == 0) dy = img_height[x][0] - img_height[x][1];
			else if (y == TEXTURE_IMAGE_HEIGHT - 2) dy = img_height[x][TEXTURE_IMAGE_HEIGHT - 2] - img_height[x][TEXTURE_IMAGE_HEIGHT - 3];
			else dy = img_height[x][y - 1] - img_height[x][y + 1];
			dy = vec3(0.0, 2.0, dy[2]);
			vec3 nor = normalize(cross(dx, dy));
			//nor = normalize(vec3((nor[0] + 1) / 2, (nor[1] + 1) / 2, (nor[2] + 1) / 2));
			img_normal[x][y] = nor;
		}
	}
